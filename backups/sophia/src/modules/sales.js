...
export class SalesModule {
    ...
    render() {
        ...
        <div class="flex flex-col h-full gap-2 lg:gap-4 animate-fade-in relative">
            <!-- Header Toolbar -->
            <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center bg-[#fff2e6] dark:bg-transparent border border-orange-100 dark:border-transparent p-3 lg:p-4 rounded-lg shadow-sm dark:shadow-none transition-colors gap-3 lg:gap-0">
                <div class="flex flex-col lg:flex-row items-stretch lg:items-center gap-2 lg:gap-4 flex-1 w-full lg:w-auto">
                    ...
                    <div class="flex gap-2 w-full lg:w-auto">
                        <select id="pos-category" class="flex-1 lg:flex-none p-2.5 bg-gray-50 dark:bg-slate-900/50 border border-gray-300 dark:border-slate-800 rounded-lg focus:ring-2 focus:ring-unitech-primary outline-none text-sm text-gray-700 dark:text-gray-200 w-full md:w-auto truncate max-w-[150px] lg:max-w-none">
                            <option value="">Todas as Categorias</option>
                        </select>

                        <div class="flex bg-gray-100 dark:bg-slate-800 p-1 rounded-lg shrink-0">
                            <div class="flex">
                                <button class="px-2 py-1.5 rounded-md text-[10px] font-bold transition-colors ${this.pricingMode === 'retail' ? 'bg-white text-unitech-primary shadow-sm dark:bg-slate-700 dark:text-white dark:shadow-none' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200'}" id="mode-retail">VAREJO</button>
                                <button class="px-2 py-1.5 rounded-md text-[10px] font-bold transition-colors ${this.pricingMode === 'wholesale' ? 'bg-white text-unitech-primary shadow-sm dark:bg-slate-700 dark:text-white dark:shadow-none' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200'}" id="mode-wholesale">ATACADO</button>
                            </div>
                            
                            <div class="w-px h-4 bg-gray-300 dark:bg-slate-600 self-center mx-1"></div>

                            <button id="btn-history" class="px-2 py-1.5 text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 rounded-md transition-colors" title="Histórico de Vendas">
            ...
            <div class="flex flex-col lg:flex-row flex-1 gap-4 lg:gap-6 overflow-hidden">
                ...
                <div class="flex-1 overflow-y-auto overscroll-contain pr-1 lg:pr-2 custom-scrollbar min-h-0 order-2 lg:order-1">
                ...
                <div class="w-full lg:w-96 bg-white border border-gray-200 rounded-lg shadow-lg flex flex-col shrink-0 order-1 lg:order-2 h-[40vh] lg:h-full lg:max-h-full">
                    <!-- Client Selector -->
    ...
    async handleWhatsAppSharing(sale) {
        ...
        if (!cleanPhone) {
            const input = prompt('Digite o celular do cliente (DDD + Número):');
            if (input) cleanPhone = input.replace(/\D/g, '');
            else return; // Cancelled
        }
        ...
        try {
            ...
            // 3. Send via WhatsApp
            if (window.WhatsAppWidget && window.WhatsAppWidget.sendMedia) {
                ...
                try {
                    await window.WhatsAppWidget.sendMedia(cleanPhone, filename, pdfBase64, caption);
        ...
        <div id="sale-details-modal" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-[70] hidden flex items-center justify-center p-4 animate-fade-in">
             <div class="bg-white rounded-2xl shadow-2xl max-w-2xl w-full overflow-hidden transform transition-all scale-100 flex flex-col max-h-[90vh]">
               ...
               <div class="flex gap-2">
                   ...
                   <button id="btn-share-whatsapp-history" class="px-4 py-2 bg-[#25D366]/10 text-green-600 font-bold rounded-lg border border-[#25D366]/20 hover:bg-[#25D366]/20 transition-colors text-xs uppercase tracking-widest flex items-center gap-2">
    ...
    async handleWhatsAppSharing(sale) {
        try {
            ...
            if (!window.whatsappService) {
                alert('Serviço de WhatsApp não disponível. Verifique a conexão.');
                return;
            }

            // Check status using the correct property
            if (window.whatsappService.status !== 'CONNECTED') {
                alert('WhatsApp não conectado. Por favor, conecte primeiro.');
                return;
            }

            // Get phone number from client or prompt
            let phone = '';
            if (sale.clientId) {
            ...
            if (!phone) {
                phone = prompt('Digite o número do WhatsApp (com DDD):');
                if (!phone) return; // User cancelled
                phone = phone.replace(/\D/g, '');
            }

            // Generate PDF receipt using jsPDF
            console.log('[SalesModule] Generating PDF receipt for WhatsApp...');
            ...
            // Set font to Courier for that thermal printer look
            ...
            doc.text('UNITECH CELULARES', 40, y, { align: 'center' });
            y += 6;

            doc.setFontSize(8);
            doc.setFont(undefined, 'normal');
            doc.text('Rua Exemplo, 123 - Centro', 40, y, { align: 'center' });
            y += 4;
            doc.text('Contato: (00) 0000-0000', 40, y, { align: 'center' });
            y += 5;
            ...
            if (osItem) {
                ...
                idDisplay = osItem.osId || (osItem.name.match(/#([0-9]+)/) || [])[1] || sale.id;
            }

            doc.setFont(undefined, 'bold');
            doc.text(`${label}: ${idDisplay}`, 5, y);
            y += 5;

            doc.setFont(undefined, 'normal');
            doc.text(`Data: ${new Date(sale.date).toLocaleString('pt-BR')}`, 5, y);
            y += 5;
            doc.text(`Vendedor: ${sale.sellerName || 'N/A'}`, 5, y);
            y += 5;
            doc.text(`Cliente: ${sale.clientName || 'Consumidor Final'}`, 5, y);
            y += 6;
            ...
            sale.items.forEach(item => {
                ...
                const nameText = item.name.substring(0, 25);
                const totalText = `R$ ${(item.total || 0).toFixed(2)}`;

                doc.text(qtyText + nameText, 5, y);
                doc.text(totalText, 75, y, { align: 'right' });
                y += 5;
                if (item.name.length > 25) {
                    doc.text(item.name.substring(25, 50), 10, y);
                    y += 5;
                }
            });
            ...
            if (sale.discount && sale.discount.value > 0) {
                ...
                doc.text(`R$ ${(sale.subtotal).toFixed(2)}`, 75, y, { align: 'right' });
                y += 5;
                doc.setFont(undefined, 'bold');
                doc.text('Desconto:', 5, y);
                doc.text(`- R$ ${parseFloat(sale.discount.amount).toFixed(2)}`, 75, y, { align: 'right' });
                y += 5;
                doc.setFont(undefined, 'normal');
                doc.text('------------------------------------------', 40, y, { align: 'center' });
                y += 5;
            ...
            doc.setFont(undefined, 'bold');
            doc.text('TOTAL:', 5, y);
            doc.text(`R$ ${(sale.total || 0).toFixed(2)}`, 75, y, { align: 'right' });
            y += 8;
            ...
            const methodMap = {
                ...
                'CREDIT_CARD_PARCELADO': 'CARTÃO DE CRÉDITO (PARC)',
                'CREDIT_CARD': 'CARTÃO DE CRÉDITO',
                'DEBIT_CARD': 'CARTÃO DE DÉBITO',
                'PIX': 'PIX', 'ORCAMENTO': 'ORÇAMENTO'
            };
            const paymentMethod = methodMap[sale.method] || sale.method || 'Dinheiro';
            doc.text(`Pagamento: ${paymentMethod}`, 5, y);
            if (sale.pixBank) {
                y += 5;
                doc.text(`Banco: ${sale.pixBank}`, 5, y);
            }
            ...
            // Send via WhatsApp
            console.log('[SalesModule] Sending receipt via WhatsApp...');
            const filename = `Comprovante_${sale.id}.pdf`;
            const caption = `*Comprovante de Venda*\n\nID: ${sale.id}\nTotal: R$ ${sale.total.toFixed(2)}\nData: ${new Date(sale.date).toLocaleString('pt-BR')}`;

            await window.whatsappService.sendMedia(phone, pdfBase64, filename, caption);

            if (window.toastService) {
                window.toastService.success('Comprovante enviado via WhatsApp!');
            } else {
                alert('Comprovante enviado via WhatsApp!');
            }
        } catch (err) {
            console.error('[SalesModule] WhatsApp sharing error:', err);
            alert('Erro ao enviar comprovante via WhatsApp: ' + err.message);
        }
    }
