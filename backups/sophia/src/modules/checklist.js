...
export class ChecklistModule {
    ...
    renderList() {
        ...
        if (this.filters.attendant) {
            filtered = filtered.filter(c => c.attendant && c.attendant.toLowerCase().includes(this.filters.attendant.toLowerCase()));
        }
        if (this.filters.priority) {
            filtered = filtered.filter(c => c.priority && c.priority.toLowerCase().includes(this.filters.priority.toLowerCase()));
        }
        if (this.filters.date) {
            filtered = filtered.filter(c => c.deadline === this.filters.date);
        }
        ...
        const totalProducts = filtered.reduce((acc, c) => acc + (c.valProd || 0), 0);
        const totalServices = filtered.reduce((acc, c) => acc + (c.valServ || 0), 0);
        // totalDiscount not really tracked per item usually, maybe diff between expected and actual? Assume 0 for now.
        const totalFinal = filtered.reduce((acc, c) => acc + (c.valTotal || 0), 0);
        ...
        // Unique Values for Datalists (Suggestions)
        // Unique Values for Datalists (Suggestions) - Fetching ALL available techs/attendants
        const uniqueClients = [...new Set(checklists.map(c => c.client).filter(Boolean))].sort();

        // Technicians: Internal (role=tech/admin/etc) + External (category=tech)
        ...
        const externalTechs = storage.getClients()
            ...
            .map(c => c.name);
        // Also include any technician already on an OS (even if deleted/archived user)
        const usedTechs = checklists.map(c => c.technician).filter(Boolean);
        const uniqueTechs = [...new Set([...internalTechs, ...externalTechs, ...usedTechs])].sort();

        // Attendants: Internal Users
        const internalAttendants = storage.getUsers().map(u => u.name);
        // Also include any attendant already on an OS
        const usedAttendants = checklists.map(c => c.attendant).filter(Boolean);
        const uniqueAttendants = [...new Set([...internalAttendants, ...usedAttendants])].sort();

        const uniqueStatuses = ['Entrada', 'Orçamento', 'Aberto', 'Garantia', 'Andamento', 'Concluído', 'Faturado', 'Finalizado', 'Realizado', 'Pendente', 'Autorizada', 'Sem Reparo'];
        ...
        return `
            <div class="flex flex-col h-full animate-fade-in space-y-2">
                ...
                <div class="bg-white p-3 rounded-lg border border-gray-200 shadow-sm flex flex-col gap-3">
                    ...
                    <!-- Filter Row with Datalists -->
                    <div class="grid grid-cols-2 md:grid-cols-6 gap-2 text-xs ${hasSelection ? 'opacity-50 pointer-events-none' : ''}">
                        
                        <!-- Status Filter -->
                        <div class="relative">
                            <select id="filter-status" class="w-full border border-gray-300 rounded px-2 py-1 text-gray-900 focus:outline-none focus:border-unitech-primary bg-white appearance-none">
                                <option value="">Status (Todos)</option>
                                ${uniqueStatuses.map(s => `<option value="${s}" ${this.filters.status === s ? 'selected' : ''}>${s}</option>`).join('')}
                            </select>
                        </div>

                        <!-- Client Filter (Keep as Input for search) -->
                         <div class="relative">
                            <input type="text" list="dl-client" id="filter-client" value="${this.filters.client || ''}" placeholder="Cliente..." class="w-full border border-gray-300 rounded px-2 py-1 text-gray-900 focus:outline-none focus:border-unitech-primary">
                            <datalist id="dl-client">
                                ${uniqueClients.map(c => `<option value="${c}">`).join('')}
                            </datalist>
                        </div>

                        <!-- Attendant Filter -->
                        <div class="relative">
                             <select id="filter-attendant" class="w-full border border-gray-300 rounded px-2 py-1 text-gray-900 focus:outline-none focus:border-unitech-primary bg-white appearance-none">
                                <option value="">Atendente (Todos)</option>
                                ${uniqueAttendants.map(a => `<option value="${a}" ${this.filters.attendant === a ? 'selected' : ''}>${a}</option>`).join('')}
                            </select>
                        </div>

                        <!-- Technician Filter -->
                        <div class="relative">
                             <select id="filter-technician" class="w-full border border-gray-300 rounded px-2 py-1 text-gray-900 focus:outline-none focus:border-unitech-primary bg-white appearance-none">
                                <option value="">Técnico (Todos)</option>
                                ${uniqueTechs.map(t => `<option value="${t}" ${this.filters.technician === t ? 'selected' : ''}>${t}</option>`).join('')}
                            </select>
                        </div>

                        <!-- Priority Filter -->
                        <div class="relative">
                             <select id="filter-priority" class="w-full border border-gray-300 rounded px-2 py-1 text-gray-900 focus:outline-none focus:border-unitech-primary bg-white appearance-none">
                         ...
                         <input type="date" id="filter-date" value="${this.filters.date || ''}" class="border border-gray-300 rounded px-2 py-1 text-gray-900 focus:outline-none focus:border-unitech-primary">
                    </div>
                </div>

                <!-- Table Container (Desktop) -->
                <div class="hidden md:flex bg-white rounded-lg border border-gray-200 shadow-sm flex-1 overflow-hidden flex-col relative">
                ...
                <!-- Mobile Cards Container (Mobile) -->
                <div class="md:hidden space-y-3 pb-20">
                    ${filtered.length > 0 ? filtered.map(os => this.renderMobileCard(os)).join('') : `
                        <div class="p-8 text-center text-gray-400 bg-white rounded-lg border border-gray-100">
    ...
    renderMobileCard(os) {
        ...
        const technician = os.technician || '---';
        ...
        // Status Badge
        const situationalStatus = os.situation || 'Pendente';
        let statusColor = 'bg-gray-100 text-gray-600 border-gray-200';
        let statusBg = 'bg-gray-50';

        const s = situationalStatus.toUpperCase();
        if (['REALIZADO', 'AUTORIZADA', 'CONCLUÍDO', 'FINALIZADO', 'ENTREGUE'].includes(s)) {
            statusColor = 'text-green-700 bg-green-50 border-green-200';
            statusBg = 'bg-green-50';
        } else if (['RECUSADA', 'SEM REPARO', 'ABANDONO', 'DESCARTE'].includes(s)) {
            statusColor = 'text-red-700 bg-red-100 border-red-200';
            statusBg = 'bg-red-50';
        } else if (['PENDENTE', 'EM ANÁLISE', 'AGUARD. PEÇA', 'ORÇAMENTO'].includes(s)) {
            statusColor = 'text-yellow-700 bg-yellow-100 border-yellow-200';
            statusBg = 'bg-yellow-50';
        }

        // Priority
        const priority = os.priority || 'Normal';
        let priorityDot = 'bg-gray-400';
        if (priority === 'Alta') priorityDot = 'bg-red-500';
        else if (priority === 'Média') priorityDot = 'bg-yellow-500';
        else if (priority === 'Baixa') priorityDot = 'bg-green-500';

        // Device Info
        const device = os.device || 'Aparelho não informado';

        return `
            <div class="bg-white rounded-xl p-4 shadow-sm border border-gray-100 relative group active:scale-[0.98] transition-all">
                <!-- Status Stripe -->
                <div class="absolute left-0 top-4 bottom-4 w-1 ${priorityDot} rounded-r-full"></div>
                
                <div class="pl-3">
                    <!-- Header -->
                    <div class="flex justify-between items-start mb-3">
                        <div>
                            <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-0.5">#${String(id).replace('CHK-', '')} • ${entryDate}</span>
                            <h4 class="font-bold text-gray-800 text-sm line-clamp-1">${client}</h4>
                        </div>
                        <span class="px-2 py-1 rounded text-[9px] font-bold uppercase tracking-wider border ${statusColor}">${situationalStatus}</span>
                    </div>

                    <!-- Body -->
                    <div class="${statusBg} rounded-lg p-2.5 mb-3 border border-gray-100/50">
                         <div class="flex items-center gap-2 mb-1">
                            <i data-feather="smartphone" class="w-3 h-3 text-gray-400"></i>
                            <span class="text-xs font-bold text-gray-700 truncate">${device}</span>
                         </div>
                         <div class="flex items-center gap-2">
                             <i data-feather="user" class="w-3 h-3 text-gray-400"></i>
                             <span class="text-[10px] text-gray-500 uppercase tracking-wide">Técnico: ${technician}</span>
                         </div>
                    </div>
                    ...
                    <div class="flex items-center justify-between border-t border-gray-100 pt-3 mt-1">
                         <div>
                            <span class="text-[9px] font-bold text-gray-400 uppercase tracking-wider block">Valor Total</span>
                            <span class="text-lg font-black text-gray-800">R$ ${valTotal}</span>
                         </div>
                         
                         <div class="flex gap-2">
                            <button class="btn-edit-os w-10 h-10 rounded-xl bg-yellow-50 text-yellow-600 flex items-center justify-center border border-yellow-100 active:scale-90 transition-transform shadow-sm" data-id="${id}">
                                <i data-feather="edit-2" class="w-4 h-4"></i>
                            </button>
                            <button class="btn-view-os w-10 h-10 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center border border-blue-100 active:scale-90 transition-transform shadow-sm" data-id="${id}">
    ...
    attachListEvents() {
        ...
        const attachFilter = (id, field) => {
            ...
            if (input) {
                ...
                input.addEventListener(eventType, (e) => {
                    ...
                    // Restore Focus and Cursor ONLY for text inputs
                    const newInput = document.getElementById(id);
                    if (newInput && newInput.tagName !== 'SELECT') {
                        newInput.focus();
        ...
        attachFilter('filter-attendant', 'attendant');
        attachFilter('filter-priority', 'priority');
        attachFilter('filter-date', 'date');
    }
                ...
                <form id="checklist-form" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                        <div class="form-group">
                            ...
                            <select id="os-origin" class="input-field w-full border border-gray-300 rounded p-2 text-sm text-gray-700 focus:ring-2 focus:ring-unitech-primary outline-none">
                                <option>----</option>
    ...
    attachCreateEvents() {
        ...
        document.getElementById('checklist-form').addEventListener('submit', async (e) => {
            ...
            try {
                ...
                const checklist = {
                    ...
                    client,
                    origin: document.getElementById('os-origin')?.value || 'Balcão',
                ...
                if (this.currentId && this.viewMode === 'edit') {
                    ...
                    const result = await storage.updateChecklist(checklist);
                    ...
                    const result = await storage.addChecklist(checklist);
        ...
        const calcTotal = () => {
            const p = parseFloat(document.getElementById('os-val-prod')?.value || 0);
            const s = parseFloat(document.getElementById('os-val-serv')?.value || 0);
            const t = document.getElementById('os-val-total');
            if (t) t.value = (p + s).toFixed(2);
        };
        document.getElementById('os-val-prod')?.addEventListener('input', calcTotal);
        document.getElementById('os-val-serv')?.addEventListener('input', calcTotal);
    ...
    populateForm() {
        ...
        if (document.getElementById('os-origin')) {
            document.getElementById('os-origin').value = os.origin || 'Balcão';
        }
        ...
        // Populate Financials
        if (document.getElementById('os-val-prod')) {
            document.getElementById('os-val-prod').value = os.valProd || '';
        }
        if (document.getElementById('os-val-serv')) {
            document.getElementById('os-val-serv').value = os.valServ || '';
        }
        if (document.getElementById('os-val-total')) {
            document.getElementById('os-val-total').value = (os.valTotal || 0).toFixed(2);
        }
