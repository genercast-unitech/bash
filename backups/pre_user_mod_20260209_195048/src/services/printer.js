...
export class PrinterService {
    ...
    async printCUPOM(saleOrId) {
        try {
            const saleId = typeof saleOrId === 'string' ? saleOrId : saleOrId.id;
            console.log(`[PrinterService] Preparando Cupom para venda ${saleId}`);
            ...
            const content = `
                <div style="font-family: monospace; padding: 20px; max-width: 300px; margin: 0 auto;">
                    ...
                    <p style="text-align: center; font-size: 12px;">Rua Exemplo, 123 - Centro<br/>Contato: (00) 0000-0000</p>
                    ...
                    if (osItem) {
                        label = sale.originQuoteId ? 'VENDA CONCRETIZADA DA OS' : 'CONSERTO DA OS';
                        if (osItem.osId) {
                    ...
                    return `<p>${label}: ${idDisplay}<br/>`;
                })()}
                    Data: ${new Date(sale.date).toLocaleString('pt-BR')}<br/>
                    Vendedor: ${sale.sellerName || 'N/A'}</p>
                    ...
                    <table style="width: 100%; font-size: 12px;">
                        ${sale.items.map(item => `
                            ...
                            <tr>
                                <td>${item.qty}x R$ ${(item.price || 0).toFixed(2)}</td>
                                <td style="text-align: right;">R$ ${(item.total || 0).toFixed(2)}</td>
                            </tr>
                        `).join('')}
                    </table>
                    ...
                    ${(sale.discount && sale.discount.value > 0) ? `
                        ...
                        <p style="display: flex; justify-content: space-between; margin: 2px 0; font-weight: bold;">
                           <span>Desconto:</span>
                           <span>- R$ ${parseFloat(sale.discount.amount).toFixed(2)}</span>
                        </p>
                         <hr style="border-top: 1px dashed #000;"/>
                ...
                })()}${sale.pixBank ? `<br/>Destino: ${sale.pixBank}` : ''}</p>
                    <hr style="border-top: 1px dashed #000;"/>
            ...
            if (window.toastService) window.toastService.error(`Erro ao imprimir Cupom: ${error.message}`);
            else alert(`Erro ao imprimir Cupom: ${error.message}`);
            return false;
        }
    }
