...
class BaseRepository {
    ...
    getAll(allowBypass = false) {
        try {
            ...
            const userStr = localStorage.getItem('unitech_current_user');
            const user = userStr ? JSON.parse(userStr) : null;
            const isMasterFlag = user && user.role === 'master';

            // Multi-Tenant Isolation Logic
            // If user is 'master' and explicitly bypasses (for management panel), return all.
            // Otherwise, filter strictly by tenantId.
            const filtered = this.data.filter(item => {
                // Master bypass for global dashboard/management
                if (isMasterFlag && allowBypass) return true;

                // Strict isolation
                return String(item.tenantId || 'master') === String(currentTenant);
            });

            return filtered.map(item => {
                const result = this.schema.safeParse(item);
                if (!result.success) {
                    console.error(`[Data Integrity Error] Non-compliant item in ${this.collectionName}:`, item.id, result.error.errors);
                    // Return it anyway but it might cause UI issues
                    return item;
                }
                return item;
            });
        } catch (error) {
            console.error(`Data Retrieval Error [${this.collectionName}]:`, error);
            return [];
        }
    }
...
// --- Tenant Repository (The SaaS Root) ---
export const TenantRepository = new BaseRepository('unitech_tenants', TenantSchema, [
    {
        id: 'master',
        name: 'UniTech Global',
        status: 'active',
...
// --- Plan Repository ---
export const PlanRepository = new BaseRepository('unitech_plans', PlanSchema, [
    {
        id: 'basic',
        name: 'BASIC',
        price: 99,
        interval: 'monthly',
        features: ['dashboard', 'sales', 'clients', 'storefront'],
        storageLimitGB: 5
    },
    {
        id: 'pro',
        ...
        price: 199,
        interval: 'monthly',
        features: ['dashboard', 'sales', 'clients', 'storefront', 'checklist', 'financial', 'warranty'],
        storageLimitGB: 10
    },
    {
        id: 'enterprise',
        ...
        price: 499,
        interval: 'monthly',
        features: ['dashboard', 'sales', 'clients', 'storefront', 'checklist', 'financial', 'warranty', 'vision', 'whatsapp', 'copilot'],
        storageLimitGB: 50
    }
]);

// --- Global SaaS Settings Repository ---
export const SassSettingsRepository = new BaseRepository('unitech_sass_settings', SassSettingsSchema, [
    {
        id: 'global',
