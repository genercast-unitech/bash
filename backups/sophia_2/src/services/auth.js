...
export class AuthService {
    ...
    /**
     * Check if user has access to a module with Tenant-Feature enforcement
    ...
    hasAccess(moduleName) {
        ...
        const tenantId = this.currentUser.tenantId || 'master';
        ...
        const allowedByRole = accessMatrix[role];
        if (!allowedByRole) return false;

        const hasRoleAccess = allowedByRole === '*' || allowedByRole.includes(moduleName);
        if (!hasRoleAccess) return false;

        // 2. Tenant-based feature control (SaaS Enforcement)
        // Master bypasses feature checks
        if (role === 'master' || this.currentUser.isImpersonated) return true;

        const tenants = TenantRepository.getAll(true);
        const tenant = tenants.find(t => t.id === tenantId);

        // Define which modules are "Global" (Always on for any plan unless explicitly blocked)
        const globalFeatures = ['dashboard', 'settings', 'copilot'];
        if (globalFeatures.includes(moduleName)) return true;

        if (tenant && tenant.enabledFeatures) {
            return tenant.enabledFeatures.includes(moduleName);
        }

        // Fallback for tenants without specific feature blocks (Default: All Access)
        return true;
    }
    ...
    async verifyCurrentPassword(password) {
        ...
        if (user.passwordHash) {
            return await encryption.verifyPassword(password, user.passwordHash);
        }
