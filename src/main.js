...
import { WhatsAppFloatingWidget } from './modules/whatsapp/components/WhatsAppFloatingWidget.js';
...
let renderApp = async (targetModule, params = {}) => {
  ...
  // Inject WhatsApp Widget (Global - Desktop Only)
  if (window.innerWidth >= 1024) {
    WhatsAppFloatingWidget.render();
  }
...
const attachGlobalListeners = () => {
  ...
  if (exitImpersonationBtn) {
    exitImpersonationBtn.addEventListener('click', () => {
      ...
      localStorage.setItem('last_module', 'master');
      window.location.reload();
    });
  }
  ...
  if (mobileMoreBtn && sidebar && drawerOverlay) {
