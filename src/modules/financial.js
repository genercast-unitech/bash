...
export class FinancialModule {
    ...
    renderMobileCards() {
        ...
        return this.getPageItems().map(t => {
                     ...
                     <div class="flex justify-end gap-2 group-hover:opacity-100 opacity-80">
                          ...
                          ` : isSale ? `
                             ...
                             <button onclick="
                                 ...
                                 const saleId = '${t.metadata?.saleId || ''}' || (match ? match[1] : '');
                                 if (saleId) {
                                     // Quick WhatsApp from row
                                     const sales = storage.getSales();
                                     const sale = sales.find(s => s.id === saleId);
                                     if (sale) window.salesModule.handleWhatsAppSharing(sale);
                                     else alert('Dados da venda não encontrados.');
                                 } else alert('ID da venda não encontrado.');
                              " class="w-8 h-8 rounded-full bg-emerald-50 text-emerald-600 hover:bg-emerald-100 flex items-center justify-center transition-colors shadow-sm" title="Enviar no WhatsApp">
                                  <i data-feather="message-circle" class="w-4 h-4"></i>
                              </button>
                          ` : `
                              <button onclick="window.financialModule.printTransaction('${t.id}')" class="w-8 h-8 rounded-full bg-gray-50 text-gray-600 hover:bg-gray-100 flex items-center justify-center transition-colors shadow-sm" title="Imprimir Recibo">
                                  <i data-feather="printer" class="w-4 h-4"></i>
                              </button>
                               <button onclick="window.financialModule.handleWhatsAppSharing('${t.id}')" class="w-8 h-8 rounded-full bg-emerald-50 text-emerald-600 hover:bg-emerald-100 flex items-center justify-center transition-colors shadow-sm" title="Enviar Recibo no WhatsApp">
                                  <i data-feather="message-circle" class="w-4 h-4"></i>
                              </button>
                          `}
                      </div>
    ...
    /**
     * Share generic transaction on WhatsApp (PDF receipt)
     */
    async handleWhatsAppSharing(id) {
        const t = this.transactions.find(item => item.id == id);
        if (!t) return;

        // 1. If it's linked to a sale, use the superior Sale PDF generator
        if (t.metadata && t.metadata.saleId) {
            const sale = (storage.getSales() || []).find(s => s.id === t.metadata.saleId);
            if (sale) {
                return window.salesModule.handleWhatsAppSharing(sale);
            }
        }

        const isRevenue = t.type === 'revenue';
        const title = isRevenue ? '💰 *RECIBO DE RECEBIMENTO*' : '💸 *COMPROVANTE DE PAGAMENTO*';
        const filename = `Recibo_${t.id.slice(-6)}.pdf`;

        // Find Phone
        let phone = '';
        const clients = (storage.getClients() || []);
        const person = clients.find(c => c.name === t.person);
        if (person) phone = person.mobile || person.phone || '';

        let cleanPhone = phone.replace(/\D/g, '');
        if (!cleanPhone) {
            const input = prompt('Digite o celular (DDD + Número):');
            if (input) cleanPhone = input.replace(/\D/g, '');
            else return;
        }

        // 2. Check for PDF libraries
        if (!window.jspdf || !window.html2canvas) {
            // Fallback to text if libraries didn't load for some reason
            let text = `${title}\n\n*Nº:* ${t.id.slice(-6)}\n*Data:* ${new Date(t.dueDate).toLocaleDateString('pt-BR')}\n*Valor:* R$ ${t.finalValue.toFixed(2)}\n\n_UniTech_`;
            if (window.WhatsAppWidget && window.WhatsAppWidget.openChat) {
                window.WhatsAppWidget.openChat(cleanPhone, t.person, text);
            }
            return;
        }
        ...
        try {
            ...
            if (!window.whatsappService || window.whatsappService.status !== 'CONNECTED') {
                const msg = '❌ WhatsApp desconectado. Por favor, conecte o WhatsApp no menu lateral.';
                if (window.toastService) window.toastService.error(msg);
                else alert(msg);
                return;
            }

            if (window.toastService) window.toastService.info('Gerando Recibo Digital...');

            const html = `
                <div style="font-family: sans-serif; padding: 30px; border: 2px solid #eee; width: 400px; background: white; color: black;">
                    <div style="text-align: center; margin-bottom: 20px;">
                        <h2 style="margin: 0; color: #1a1a1a;">UNITECH</h2>
                        <p style="font-size: 12px; color: #666; margin: 5px 0;">Comprovante Digital de Transação</p>
                    </div>
                    
                    <div style="background: #f9f9f9; padding: 15px; border-radius: 8px; margin-bottom: 20px;">
                        <h3 style="margin: 0; font-size: 16px; color: ${isRevenue ? '#10b981' : '#ef4444'}; text-align: center;">
                            R$ ${t.finalValue.toFixed(2)}
                        </h3>
                    </div>

                    <div style="font-size: 13px; line-height: 1.6;">
                        <p><strong>Nº Controle:</strong> ${t.id.slice(-8).toUpperCase()}</p>
                        <p><strong>Data:</strong> ${new Date(t.dueDate).toLocaleDateString('pt-BR')}</p>
                        <p><strong>Tipo:</strong> ${isRevenue ? 'Entrada / Recebimento' : 'Saída / Pagamento'}</p>
                        <p><strong>${isRevenue ? 'Cliente' : 'Favorecido'}:</strong> ${t.person}</p>
                        <p><strong>Descrição:</strong> ${t.description}</p>
                        <p><strong>Categoria:</strong> ${t.category}</p>
                        <p><strong>Meio de Pagamento:</strong> ${t.method}</p>
                    </div>

                    <div style="margin-top: 30px; padding-top: 15px; border-top: 1px solid #eee; text-align: center; font-size: 11px; color: #999;">
                        Autenticado via UniTech System<br/>${new Date().toLocaleString('pt-BR')}
                    </div>
                </div>
            `;
            ...
            if (window.WhatsAppWidget && window.WhatsAppWidget.sendMedia) {
                try {
                    await window.WhatsAppWidget.sendMedia(cleanPhone, filename, pdfBase64, `${title}\n*Valor:* R$ ${t.finalValue.toFixed(2)}`);
                    if (window.toastService) window.toastService.success('Recibo enviado com sucesso!');
                } catch (sendError) {
                    let userMsg = 'Erro ao enviar WhatsApp.';
                    if (sendError.message === 'INVALID_NUMBER') {
                        userMsg = '❌ Ops! Este número não possui WhatsApp ou é inválido. Verifique os dados do cliente.';
                    } else if (sendError.message === 'WHATSAPP_DISCONNECTED') {
                        userMsg = '❌ WhatsApp desconectado. Reconecte e tente novamente.';
                    } else {
                        userMsg = `❌ Erro: ${sendError.message}`;
                    }
            ...
            console.error('Financial WhatsApp Error:', e);
            alert('Falha ao processar comprovante PDF.');
        }
    }
    ...
    renderPagination() {
        ...
        if (totalItems <= this.itemsPerPage) return ''; // No pagination needed

        return `
            <div class="flex items-center justify-between px-4 py-3 bg-white rounded-xl border border-gray-100 shadow-sm mt-2">
                <span class="text-xs font-bold text-gray-500">Mostrando ${start}-${end} de ${totalItems}</span>
                <div class="flex items-center gap-2">
                    <button onclick="window.financialModule.goToPage(1)" 
                        class="w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold transition-colors ${this.currentPage === 1 ? 'bg-gray-100 text-gray-300 cursor-not-allowed' : 'bg-gray-50 text-gray-600 hover:bg-gray-200'}" 
                        ${this.currentPage === 1 ? 'disabled' : ''}>
                        <i data-feather="chevrons-left" class="w-4 h-4"></i>
                    </button>
                    <button onclick="window.financialModule.goToPage(${this.currentPage - 1})" 
                        class="w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold transition-colors ${this.currentPage === 1 ? 'bg-gray-100 text-gray-300 cursor-not-allowed' : 'bg-gray-50 text-gray-600 hover:bg-gray-200'}" 
                        ${this.currentPage === 1 ? 'disabled' : ''}>
                        <i data-feather="chevron-left" class="w-4 h-4"></i>
                    </button>
                    <span class="px-3 py-1 bg-gray-900 text-white text-xs font-black rounded-lg">${this.currentPage} / ${total}</span>
                    <button onclick="window.financialModule.goToPage(${this.currentPage + 1})" 
                        class="w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold transition-colors ${this.currentPage === total ? 'bg-gray-100 text-gray-300 cursor-not-allowed' : 'bg-gray-50 text-gray-600 hover:bg-gray-200'}" 
                        ${this.currentPage === total ? 'disabled' : ''}>
                        <i data-feather="chevron-right" class="w-4 h-4"></i>
                    </button>
                    <button onclick="window.financialModule.goToPage(${total})" 
                        class="w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold transition-colors ${this.currentPage === total ? 'bg-gray-100 text-gray-300 cursor-not-allowed' : 'bg-gray-50 text-gray-600 hover:bg-gray-200'}" 
                        ${this.currentPage === total ? 'disabled' : ''}>
