...
export class ClientModule {
    ...
    render() {
        ...
        <div class="flex flex-col h-full animate-fade-in relative z-0">
            <!-- Top Controls Bar -->
            <div class="bg-white p-2 md:p-3 rounded-t-lg border-b border-gray-200 flex flex-col md:flex-row gap-2 md:items-center justify-between shadow-sm sticky top-0 z-20">
                
                <!-- Mobile View: Search + Plus -->
                <div class="md:hidden flex items-center gap-2 w-full">
                    <div class="relative flex-1 group">
                        <i data-feather="search" class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 group-focus-within:text-unitech-primary"></i>
                        <input type="text" id="client-search-mobile" placeholder="Item, valor ou código" class="w-full h-11 pl-10 pr-4 bg-gray-50/50 border border-gray-100 rounded-xl text-sm text-gray-800 placeholder-gray-400 focus:bg-white focus:ring-4 focus:ring-unitech-primary/5 focus:border-unitech-primary outline-none transition-all shadow-inner">
                    </div>
                    <button id="btn-new-client-mobile" class="w-11 h-11 flex items-center justify-center bg-white border border-gray-100 text-emerald-500 rounded-xl active:scale-95 transition-all shadow-sm">
                ...
                <div class="hidden md:flex items-center gap-3 w-full">
                    <button id="btn-new-client" class="bg-unitech-primary hover:bg-slate-900 active:scale-95 text-white font-black h-10 px-6 rounded-xl text-[10px] flex items-center gap-2 transition-all shadow-md shadow-unitech-primary/20 uppercase tracking-widest group shrink-0">
                    ...
                    <div class="flex-1 flex gap-2">
                        <div class="relative group flex-1">
                            <i data-feather="search" class="w-3.5 h-3.5 absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 group-focus-within:text-unitech-primary"></i>
                            <input type="text" id="client-search" value="${this.filters.search || ''}" placeholder="Nome, CPF/CNPJ, Telefone..." class="w-full h-10 pl-10 pr-4 bg-gray-50/50 border border-gray-100 rounded-xl text-xs text-gray-800 placeholder-gray-400 focus:bg-white focus:ring-4 focus:ring-unitech-primary/5 focus:border-unitech-primary outline-none transition-all">
                        </div>
                        
                        <select id="filter-client-type" class="bg-gray-50/50 border border-gray-100 rounded-xl h-10 px-4 text-xs text-gray-700 font-bold focus:ring-4 focus:ring-unitech-primary/5 outline-none transition-all appearance-none cursor-pointer">
                    ...
                    <div class="flex items-center gap-1 border-l border-gray-200 pl-2">
                         <button id="btn-print-clients" class="p-2.5 text-gray-400 hover:text-slate-900 hover:bg-gray-50 rounded-xl transition-all" title="Imprimir"><i data-feather="printer" class="w-4 h-4"></i></button>
                         <button id="btn-export-clients" class="p-2.5 text-gray-400 hover:text-slate-900 hover:bg-gray-50 rounded-xl transition-all" title="Exportar"><i data-feather="download" class="w-4 h-4"></i></button>
                         <button class="bg-slate-900 hover:bg-black text-white h-10 w-10 flex items-center justify-center rounded-xl transition-all shadow-md shrink-0 ml-1"><i data-feather="filter" class="w-4 h-4"></i></button>
                    </div>
                </div>

                <!-- Bulk Selection Action (Absolute overlap or row?) -->
                ${hasSelection ? `
                    <div class="absolute inset-0 bg-white md:bg-gray-50/95 flex items-center px-4 gap-4 animate-fade-in z-30">
                        <span class="text-xs font-black text-slate-400 uppercase tracking-widest">${this.selectedIds.size} Selecionados</span>
                        <div class="flex-1"></div>
                        <button id="btn-bulk-delete" class="bg-rose-500 hover:bg-rose-600 text-white font-black py-2.5 px-6 rounded-xl text-[10px] uppercase shadow-lg shadow-rose-500/20 flex items-center gap-2 transition-all">
                            <i data-feather="trash-2" class="w-4 h-4"></i> Excluir Registros
                        </button>
                    </div>
                ` : ''}
            </div>
    ...
    attachEvents() {
        ...
        const searchInputMobile = document.getElementById('client-search-mobile');

        const handleSearchInput = (val, source) => {
            this.filters.search = val;
            this.render();
            // Sync
            if (source === 'desktop' && searchInputMobile) searchInputMobile.value = val;
            if (source === 'mobile' && searchInput) searchInput.value = val;

            // Maintain focus on the correct input after render
            const activeInp = document.getElementById(source === 'mobile' ? 'client-search-mobile' : 'client-search');
            if (activeInp) {
                activeInp.focus();
                try {
                    activeInp.setSelectionRange(activeInp.value.length, activeInp.value.length);
                } catch (e) { }
            }
        };
        ...
        if (searchInputMobile) {
            searchInputMobile.addEventListener('input', (e) => handleSearchInput(e.target.value, 'mobile'));
        }
        ...
        const btnNewMobile = document.getElementById('btn-new-client-mobile');
        ...
        if (btnNew) btnNew.addEventListener('click', handleNewClient);
        if (btnNewMobile) btnNewMobile.addEventListener('click', handleNewClient);
