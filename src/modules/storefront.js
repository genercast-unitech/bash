...
export class StorefrontModule {
    ...
    async init(containerId, params = {}) {
        ...
        this.fixData();
        this.products = storage.getProducts().filter(p => p && p.name && p.name.trim());
        this.clients = storage.getClients();
        this.categories = storage.getCategories();
        this.locations = storage.getLocations();

        container.innerHTML = `
            <div class="flex flex-col gap-1 md:gap-6 animate-fade-in text-white p-1 md:p-4 pb-24">
                <!-- Header / Filter Bar -->
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 flex flex-col flex-shrink-0">
                    <!-- Mobile View: Ultra Compact Toolbar -->
                    <div class="md:hidden flex items-center gap-1.5 p-1 h-10 w-full overflow-hidden">
                        <input type="text" id="search-name-mobile" placeholder="BUSCAR..." class="flex-[1.3] h-8 px-2 bg-gray-50 border border-gray-100 rounded text-[8px] font-medium text-gray-700 placeholder-gray-400 focus:bg-white focus:border-unitech-primary outline-none transition-all uppercase m-0 min-w-0">

                        <div class="relative flex-1 h-8 min-w-0">
                            <select id="filter-category-mobile" class="w-full h-8 px-4 bg-gray-50 border border-gray-100 rounded text-[8px] font-medium text-slate-500 outline-none appearance-none cursor-pointer uppercase tracking-tight focus:bg-white focus:border-unitech-primary transition-all m-0 text-center">
                                <option value="">TODAS</option>
                                ${this.renderCategoryOptions()}
                            </select>
                            <i data-feather="chevron-down" class="absolute right-1 top-1/2 -translate-y-1/2 w-2 h-2 text-gray-400 pointer-events-none"></i>
                        </div>

                        <button id="btn-new-product-mobile" class="w-8 h-8 flex items-center justify-center bg-unitech-primary text-white rounded active:scale-95 transition-all shrink-0 m-0">
                            <i data-feather="plus" class="w-4 h-4"></i>
                        </button>
                    </div>

                    <!-- Desktop View: Professional Grid -->
                    <div class="hidden md:flex flex-row items-center justify-between gap-3 p-3">
                        <button id="btn-new-product" class="bg-unitech-primary hover:bg-slate-900 active:scale-[0.98] text-white font-black h-10 px-6 rounded-xl text-[10px] flex items-center gap-2 transition-all shadow-md shadow-unitech-primary/20 uppercase tracking-widest group shrink-0">
                            <i data-feather="plus-circle" class="w-4 h-4 group-hover:rotate-90 transition-transform"></i> Novo Registro
                        </button>
                        ...
                        <div class="flex gap-2 flex-1">
                            <div class="relative group flex-1">
                                <i data-feather="search" class="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400 group-focus-within:text-unitech-primary"></i>
                                <input type="text" id="search-name" placeholder="Buscar por nome, categoria ou fornecedor..." class="w-full h-10 pl-10 pr-4 bg-gray-50/50 border border-gray-100 rounded-xl text-xs text-gray-800 placeholder-gray-400 focus:ring-4 focus:ring-unitech-primary/5 focus:bg-white focus:border-unitech-primary outline-none transition-all">
                            </div>

                            <div class="relative group w-40">
                                <i data-feather="maximize" class="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400"></i>
                                <input type="text" id="search-barcode" placeholder="SKU/EAN" class="w-full h-10 pl-10 pr-4 bg-gray-50/50 border border-gray-100 rounded-xl text-xs text-gray-800 placeholder-gray-400 focus:ring-4 focus:ring-unitech-primary/5 focus:bg-white focus:border-unitech-primary outline-none transition-all font-mono uppercase">
                            </div>
                        </div>

                        <div class="flex gap-2 items-center">
                            <select id="filter-supplier" class="bg-gray-50/50 border border-gray-100 rounded-xl h-10 px-4 text-xs text-gray-700 font-bold focus:ring-4 focus:ring-unitech-primary/5 outline-none transition-all appearance-none cursor-pointer">
                                <option value="">Fornecedor</option>
                                ${this.renderSupplierOptions()}
                            </select>

                            <select id="filter-category" class="bg-gray-50/50 border border-gray-100 rounded-xl h-10 px-4 text-xs text-gray-700 font-bold focus:ring-4 focus:ring-unitech-primary/5 outline-none transition-all appearance-none cursor-pointer">
                                <option value="">Categoria</option>
                                ${this.renderCategoryOptions()}
                            </select>

                            <button id="btn-apply-filters" class="bg-slate-900 hover:bg-black active:scale-[0.98] text-white h-10 w-10 flex items-center justify-center rounded-xl transition-all shadow-md shrink-0">
                ...
                <!-- Mobile Cards (Mobile) -->
                <div id="product-list-mobile" class="md:hidden space-y-2 mb-10">
    ...
    renderMobileCards() {
        ...
        return this.products.map(p => {
            const stockColor = p.stock > (p.minStock || 5) ? 'text-emerald-600' : 'text-rose-600';
            return `
            <div class="bg-white rounded-lg p-1.5 shadow-sm border border-gray-100 flex gap-2.5 items-center relative active:scale-[0.98] transition-all">
                <!-- Product Photo (w-12) -->
                <div class="w-12 h-12 rounded-md bg-gray-50 border border-gray-100 shrink-0 overflow-hidden flex items-center justify-center">
                    ${p.photo ? `<img src="${p.photo}" class="w-full h-full object-cover">` : `<i data-feather="image" class="w-5 h-5 text-gray-300"></i>`}
                </div>
                
                <div class="flex-1 min-w-0 flex flex-col justify-center gap-0.5">
                    <h4 class="font-black text-slate-800 text-[11px] uppercase tracking-tight truncate">${p.name}</h4>
                    
                    <div class="flex items-center gap-1.5 mt-0.5 min-w-0">
                        <!-- Info Flow: Price, Stock, Category -->
                        <span class="text-[10px] font-black text-slate-900 whitespace-nowrap shrink-0">R$ ${Number(p.retail || 0).toFixed(2)}</span>
                        <span class="text-[8px] font-bold ${stockColor} whitespace-nowrap shrink-0 border-l border-gray-200 pl-1.5">E: ${p.stock}</span>
                        <span class="text-[8px] font-medium text-slate-400 uppercase truncate max-w-[50px] shrink-0">${p.category || 'Geral'}</span>

                        <!-- Action Icons: Packed after category, reduced square size (w-5), increased icon size (w-3) -->
                        <div class="flex gap-1 shrink-0 items-center ml-0.5">
                            <button class="btn-share-catalog w-5 h-5 flex items-center justify-center rounded bg-emerald-50 text-emerald-600 border border-emerald-100" data-id="${p.id}"><i data-feather="share-2" class="w-3 h-3 pointer-events-none"></i></button>
                            <button class="btn-edit w-5 h-5 flex items-center justify-center rounded bg-blue-50 text-blue-500 border border-blue-100" data-id="${p.id}"><i data-feather="edit-2" class="w-3 h-3 pointer-events-none"></i></button>
                            <button class="btn-delete w-5 h-5 flex items-center justify-center rounded bg-rose-50 text-rose-500 border border-rose-100" data-id="${p.id}"><i data-feather="trash-2" class="w-3 h-3 pointer-events-none"></i></button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        return `
            <div id="product-modal" class="hidden fixed inset-0 z-[100] flex items-center justify-center bg-gray-900/50 backdrop-blur-sm p-4 transition-all duration-300">
                <div class="bg-white rounded-3xl shadow-2xl w-full max-w-5xl overflow-hidden animate-scale-in flex flex-col h-full md:h-auto md:max-h-[85vh] ring-1 ring-gray-100">
                    ...
                    <!-- Header -->
                    <div class="flex justify-between items-center px-4 py-4 md:px-8 md:py-6 border-b border-gray-100 bg-white sticky top-0 z-20">
                        <div>
                            <h3 class="text-lg md:text-2xl font-black text-slate-900 tracking-tight"><span>Novo Produto</span></h3>
                            <p class="text-[8px] md:text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5 md:mt-1">Gestão de Inventário</p>
                        </div>
                        <button id="close-intro-modal" class="w-8 h-8 md:w-10 md:h-10 flex items-center justify-center rounded-xl hover:bg-slate-50 text-slate-400 hover:text-slate-600 transition-all active:scale-95">
                            <i data-feather="x" class="w-4 h-4 md:w-5 md:h-5"></i>
                        </button>
                    </div>
                    
                    <!-- Body -->
                    <div class="p-4 md:p-8 overflow-y-auto custom-scrollbar flex-1 bg-white">
                        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                            ...
                            <!-- Right: Financials -->
                            <div class="lg:col-span-5 space-y-6 md:space-y-8 bg-gray-50/50 p-4 md:p-6 rounded-2xl md:rounded-3xl border border-gray-100 h-fit mb-6 md:mb-0">
                                <div class="space-y-6">
                                    <div class="flex items-center gap-2 mb-4">
    ...
    render() {
        ...
        const mobileList = document.getElementById('product-list-mobile');
        if (mobileList) mobileList.innerHTML = this.renderMobileCards();
    ...
    attachStaticEvents() {
        ...
        const searchNameMobile = document.getElementById('search-name-mobile');
        ...
        const handleSearchNameInput = (e) => {
            ...
            // Sync both inputs
            if (searchName && e.target !== searchName) searchName.value = val;
            if (searchNameMobile && e.target !== searchNameMobile) searchNameMobile.value = val;
        };
        ...
        if (searchName && !searchName.dataset.hasListener) {
            ...
            searchName.addEventListener('input', (e) => {
                ...
                debounceName = setTimeout(() => handleSearchNameInput(e), 300);
            });
        }

        if (searchNameMobile && !searchNameMobile.dataset.hasListener) {
            searchNameMobile.dataset.hasListener = 'true';
            let debounceNameMobile;
            searchNameMobile.addEventListener('input', (e) => {
                clearTimeout(debounceNameMobile);
                debounceNameMobile = setTimeout(() => handleSearchNameInput(e), 150); // Faster for mobile
            });
        }
        ...
        const filterSupplierMobile = document.getElementById('filter-supplier-mobile');
        const filterCategoryMobile = document.getElementById('filter-category-mobile');
        const btnToggleFiltersMobile = document.getElementById('btn-toggle-filters-mobile');

        const syncFilters = (type, val) => {
            this.filters[type] = val;
            this.applyFilters();

            if (type === 'supplier') {
                if (filterSupplier) filterSupplier.value = val;
                if (filterSupplierMobile) filterSupplierMobile.value = val;
            } else if (type === 'category') {
                if (filterCategory) filterCategory.value = val;
                if (filterCategoryMobile) filterCategoryMobile.value = val;
            }
        };

        if (filterSupplier && !filterSupplier.dataset.hasListener) {
            filterSupplier.dataset.hasListener = 'true';
            filterSupplier.addEventListener('change', (e) => syncFilters('supplier', e.target.value));
        }
        if (filterSupplierMobile && !filterSupplierMobile.dataset.hasListener) {
            filterSupplierMobile.dataset.hasListener = 'true';
            filterSupplierMobile.addEventListener('change', (e) => syncFilters('supplier', e.target.value));
        }

        if (filterCategory && !filterCategory.dataset.hasListener) {
            filterCategory.dataset.hasListener = 'true';
            filterCategory.addEventListener('change', (e) => syncFilters('category', e.target.value));
        }
        if (filterCategoryMobile && !filterCategoryMobile.dataset.hasListener) {
            filterCategoryMobile.dataset.hasListener = 'true';
            filterCategoryMobile.addEventListener('change', (e) => syncFilters('category', e.target.value));
        }
        ...
        const btnNewMobile = document.getElementById('btn-new-product-mobile');
        ...
        if (btnNew && !btnNew.dataset.hasListener) {
            ...
            btnNew.addEventListener('click', handleNewProduct);
        }

        if (btnNewMobile && !btnNewMobile.dataset.hasListener) {
            btnNewMobile.dataset.hasListener = 'true';
            btnNewMobile.addEventListener('click', handleNewProduct);
        }
    ...
    attachDynamicEvents() {
        ...
        const handleProductClick = (e) => {
            ...
            const btnShare = e.target.closest('.btn-share-catalog');
        ...
        if (productListMobile && !productListMobile.dataset.hasDelegate) {
            productListMobile.dataset.hasDelegate = 'true';
            productListMobile.addEventListener('click', handleProductClick);
        }
