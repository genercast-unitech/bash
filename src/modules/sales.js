...
export class SalesModule {
    ...
    updateCategories() {
        ...
        const selectMobile = document.getElementById('pos-category-mobile');
        if (!select && !selectMobile) return;

        const categories = storage.getCategories();
        if (categories && categories.length > 0) {
            const options = categories.map(c => `<option value="${c.name}">${c.name}</option>`).join('');
            const fullOptions = `<option value="">Todas as Categorias</option>${options}`;
            if (select) select.innerHTML = fullOptions;
            if (selectMobile) selectMobile.innerHTML = fullOptions;
        }
    }

    render() {
        this.container.innerHTML = `
        <div class="flex flex-col h-full gap-2 lg:gap-4 animate-fade-in relative">
            <!-- Header Toolbar -->
            <div class="flex flex-col bg-white dark:bg-transparent border-b border-gray-100 dark:border-slate-800 p-0 rounded-t-lg transition-colors overflow-hidden">
                <!-- Mobile View: Tabs + Compact Search -->
                <div class="lg:hidden flex flex-col gap-2 p-2">
                    <div class="flex border-b border-gray-100 flex-nowrap overflow-x-auto no-scrollbar -mx-2 px-2">
                        <button class="flex-1 min-w-[80px] pb-2 text-[10px] font-black text-unitech-primary border-b-2 border-unitech-primary uppercase tracking-widest transition-all">Itens</button>
                        <button class="flex-1 min-w-[80px] pb-2 text-[10px] font-bold text-gray-400 uppercase tracking-widest transition-all">Estoque</button>
                        <button class="flex-1 min-w-[80px] pb-2 text-[10px] font-bold text-gray-400 uppercase tracking-widest transition-all">Categorias</button>
                    </div>
                    
                    <div class="flex items-center gap-2 mt-1">
                        <div class="relative flex-1 group">
                            <i data-feather="search" class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 transition-colors group-focus-within:text-unitech-primary"></i>
                            <input type="text" id="pos-search-mobile" placeholder="Buscar produto..." class="w-full h-10 pl-10 pr-4 bg-gray-50/50 border border-gray-100 rounded-xl text-sm text-gray-800 placeholder-gray-400 focus:bg-white focus:ring-4 focus:ring-unitech-primary/5 focus:border-unitech-primary outline-none transition-all shadow-inner uppercase font-mono">
                        </div>
                        <button id="btn-toggle-filters-pos-mobile" class="w-10 h-10 flex items-center justify-center bg-white border border-gray-100 text-slate-600 rounded-xl active:scale-95 transition-all shadow-sm">
                            <i data-feather="filter" class="w-4 h-4"></i>
                        </button>
                        <button id="btn-new-product-pos-mobile" class="w-10 h-10 flex items-center justify-center bg-[#22c55e] text-white rounded-xl active:scale-95 transition-all shadow-md shadow-green-500/20">
                            <i data-feather="plus" class="w-5 h-5"></i>
                        </button>
                    </div>

                    <!-- Mobile Expandable Filters -->
                    <div id="pos-mobile-filters-container" class="hidden animate-slide-down">
                        <div class="pt-1">
                            <select id="pos-category-mobile" class="w-full h-11 px-4 bg-gray-50/50 border border-gray-100 rounded-xl text-sm font-bold text-gray-700 outline-none transition-all appearance-none cursor-pointer">
                                <option value="">Todas as Categorias</option>
                                ${this.renderCategoryOptions ? this.renderCategoryOptions() : ''}
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <!-- Desktop Toolbar -->
                <div class="hidden lg:flex flex-row justify-between items-center p-4 gap-4">
                    <div class="flex items-center gap-4 flex-1">
                        <div class="relative flex-1">
                            ...
                            <input type="text" id="pos-search" 
                                class="pl-10 w-full p-2.5 bg-gray-50 dark:bg-slate-900/50 border border-gray-300 dark:border-slate-800 rounded-lg focus:ring-2 focus:ring-unitech-primary focus:border-unitech-primary outline-none transition-all uppercase font-mono text-sm text-gray-900 dark:text-white placeholder-gray-500" 
                                placeholder="BUSCAR PRODUTO (NOME, SKU, TAGS)...">
                        </div>
                        
                        <div class="flex gap-2 shrink-0">
                            <select id="pos-category" class="p-2.5 bg-gray-50 dark:bg-slate-900/50 border border-gray-300 dark:border-slate-800 rounded-lg focus:ring-2 focus:ring-unitech-primary outline-none text-sm text-gray-700 dark:text-gray-200 truncate min-w-[180px]">
                                <option value="">Todas as Categorias</option>
                            </select>
    
                            <div class="flex bg-gray-100 dark:bg-slate-800 p-1 rounded-lg shrink-0">
                                <div class="flex">
                                    <button class="px-3 py-1.5 rounded-md text-[10px] font-black transition-all ${this.pricingMode === 'retail' ? 'bg-white text-unitech-primary shadow-sm' : 'text-gray-500 hover:text-gray-700'}" id="mode-retail">VAREJO</button>
                                    <button class="px-3 py-1.5 rounded-md text-[10px] font-black transition-all ${this.pricingMode === 'wholesale' ? 'bg-white text-unitech-primary shadow-sm' : 'text-gray-500 hover:text-gray-700'}" id="mode-wholesale">ATACADO</button>
                                </div>
                                <div class="w-px h-4 bg-gray-300 dark:bg-slate-600 self-center mx-1"></div>
                                <button id="btn-history" class="px-3 py-1.5 text-gray-500 hover:text-gray-900 rounded-md transition-all" title="Histórico de Vendas">
            ...
            <div class="flex flex-col lg:flex-row flex-1 gap-4 lg:gap-6 overflow-hidden">
                <!-- Product Grid (Main Area) -->
                <div class="flex-1 overflow-y-auto overscroll-contain pr-1 lg:pr-2 custom-scrollbar min-h-0">
                    <div id="pos-grid" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-5 gap-2 md:gap-4 content-start pb-24 lg:pb-0">
                        ${this.renderProductGrid()}
                    </div>
                </div>

                <!-- Cart Panel (Right/Bottom) - Hidden on mobile, uses FAB + Modal instead -->
                <div class="hidden lg:flex lg:w-96 bg-white border border-gray-200 rounded-lg shadow-lg flex-col shrink-0 h-full max-h-full">
                    <!-- Client Selector -->
                    <div class="p-3 lg:p-4 border-b border-gray-100 bg-gray-50 rounded-t-lg shrink-0">
                        <div class="flex items-center justify-between mb-2">
                            <label class="text-sm font-bold text-black uppercase tracking-wider">Cliente</label>
                            <div class="flex gap-2">
                                <button id="btn-share-cart-whatsapp" class="text-emerald-500 hover:bg-emerald-50 p-1.5 rounded-lg transition-all active:scale-95 flex items-center gap-1" title="Compartilhar Carrinho via WhatsApp">
                    ...
                    <!-- Cart Items -->
                    <div class="flex-1 overflow-y-auto p-3 lg:p-4 space-y-3 min-h-0 relative" id="cart-container">
                        ${this.renderCartItems()}

                        <!-- WhatsApp Floating Button Slot -->
                        <div id="pos-whatsapp-fab-slot" class="absolute bottom-4 right-4 pointer-events-auto z-40"></div>
                    </div>

                    <!-- Totals & Actions -->
                    <div class="px-3 lg:px-4 py-3 bg-gray-50 border-t border-gray-200 rounded-b-lg space-y-3 shrink-0">
        ...
        <div id="checkout-modal" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 hidden flex items-center justify-center animate-fade-in p-0 md:p-4">
             <div class="bg-white rounded-none md:rounded-xl shadow-2xl max-w-md w-full overflow-hidden transform transition-all scale-100 flex flex-col h-full md:h-auto md:max-h-[90vh]">
                  <div class="bg-gray-50 p-4 md:p-5 border-b border-gray-100 flex justify-between items-center shrink-0">
                  ...
                  <div class="p-4 md:p-6 overflow-y-auto flex-1 custom-scrollbar">
                      <div class="grid grid-cols-2 gap-3 md:gap-4" id="checkout-methods-grid">
         ...
         <div id="quick-client-modal" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-[60] hidden flex items-center justify-center p-0 md:p-4 animate-fade-in">
             <div class="bg-white rounded-none md:rounded-2xl shadow-2xl max-w-2xl w-full overflow-hidden transform transition-all scale-100 flex flex-col h-full md:h-auto md:max-h-[90vh]">
                 <div class="bg-gray-50 p-4 md:p-5 border-b border-gray-100 flex justify-between items-center shrink-0">
                     <div>
                        <h3 class="font-black text-lg md:text-xl text-gray-800 uppercase tracking-tight">Cadastrar Novo Cliente</h3>
                        <p class="text-[9px] md:text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-0.5">Preencha os dados completos abaixo</p>
                     </div>
                 ...
                 <div class="p-4 md:p-8 overflow-y-auto flex-1 custom-scrollbar space-y-4 md:space-y-6">
              ...
              <div class="bg-white h-full w-full max-w-md flex flex-col shadow-2xl transform transition-all translate-x-0">
                  ...
                  <div class="p-3 bg-gray-50 border-b border-gray-100">
                      <div class="flex items-center justify-between mb-2">
                          ...
                          <div class="flex gap-2">
                               <button id="btn-share-cart-whatsapp-mobile" class="text-emerald-500 hover:bg-emerald-50 px-1.5 py-0.5 rounded-md transition-all active:scale-95 flex items-center gap-1 text-xs font-bold" title="Compartilhar via WhatsApp">
                                   <i data-feather="share-2" class="w-3.5 h-3.5"></i>
                                   <span>Catálogo</span>
                               </button>
        ...
        setTimeout(() => {
            ...
            // Re-inject WhatsApp FAB if available
            const fabSlot = document.getElementById('pos-whatsapp-fab-slot');
            if (fabSlot && window.WhatsAppWidget && window.WhatsAppWidget.embedInto) {
                window.WhatsAppWidget.embedInto(fabSlot);
            }
        }, 0);
    }
    ...
    renderProductGrid() {
        ...
        return this.filteredProducts.map(p => {
            ...
            <div class="product-card group relative bg-white border border-gray-200 rounded-lg overflow-hidden hover:shadow-lg transition-all cursor-pointer ${outOfStock && !p.isOS ? 'opacity-60 grayscale' : ''}" 
                ...
                <div class="h-28 md:h-auto md:aspect-square bg-gray-50 relative overflow-hidden">
            ...
            if (success) {
                ...
                sale.items.forEach(item => {
                    if (item.isOS && item.osId) {
                        ...
                        if (os) {
                            os.situation = 'Entregue';
                            os.status = 'Entregue'; // Update legacy status too
                            storage.updateChecklist(os);
    ...
    /**
     * Share all items currently in the cart as a professional catalog message with PHOTOS
     */
    async shareCartCatalog() {
        if (!this.cart || this.cart.length === 0) {
            const msg = '❌ O carrinho está vazio! Adicione itens antes de compartilhar o catálogo.';
            if (window.toastService) window.toastService.warning(msg);
            else alert(msg);
            return;
        }

        if (!window.whatsappService || window.whatsappService.status !== 'CONNECTED') {
            const msg = '⚠️ WhatsApp não está conectado. Conecte primeiro para enviar fotos.';
            if (window.toastService) window.toastService.warning(msg);
            else alert(msg);
            return;
        }
        ...
        let phone = '';
        let clientName = this.selectedClient ? this.selectedClient.name : 'Cliente';

        if (this.selectedClient) {
            phone = this.selectedClient.mobile || this.selectedClient.phone || '';
        }

        let cleanPhone = (phone || '').replace(/\D/g, '');

        if (!cleanPhone) {
            const input = prompt('Digite o WhatsApp do cliente para enviar o catálogo:', phone);
            if (input) cleanPhone = input.replace(/\D/g, '');
            else return; // Cancelled
        }

        if (window.toastService) window.toastService.info('Preparando catálogo com fotos para o cliente...');

        try {
            // Send products one by one with their respective photos
            for (const item of this.cart) {
                const photo = item.photo || '';
                const priceStr = formatCurrency(this.pricingMode === 'retail' ? item.retail : item.wholesale);

                const caption = `🚀 *NOVIDADE NA UNITECH*\n\n` +
                    `📦 *PRODUTO:* ${item.name.toUpperCase()}\n` +
                    `${item.brand ? `🏷️ *MARCA:* ${item.brand.toUpperCase()}\n` : ''}` +
                    `💰 *VALOR:* ${priceStr}\n\n` +
                    `${item.specifications ? `📝 *Detalhes:* ${item.specifications}\n` : ''}`;

                if (photo && photo.startsWith('data:image')) {
                    // Send as media if it's base64/dataURL
                    await window.whatsappService.sendMedia(cleanPhone, photo, `${item.id}.jpg`, caption);
                } else if (photo && (photo.startsWith('http') || photo.startsWith('https'))) {
                    // Send as media with URL (Backend must support URL or we must convert)
                    // Let's convert to base64 if it's a URL to be safe
                    try {
                        ...
                        await window.whatsappService.sendMedia(cleanPhone, base64, `${item.id}.jpg`, caption);
                    } catch (fetchErr) {
                        console.error('Failed to fetch image for WhatsApp sharing:', fetchErr);
                        await window.whatsappService.sendMessage(cleanPhone, caption);
                    }
                } else {
            ...
            if (window.toastService) window.toastService.success(`✅ ${this.cart.length} produto(s) enviado(s) com sucesso!`);
        } catch (error) {
            console.error('[ShareCartCatalog] Error:', error);
            if (window.toastService) window.toastService.error('Erro ao enviar catálogo.');
        }
    }

    /**
     * Centralized WhatsApp Sharing Logic
     * @param {Object} sale - Sale or Quote object
     */
    async handleWhatsAppSharing(sale) {
        let phone = '';
        let clientName = sale.clientName;

        // Tenta encontrar o cliente pelo ID salvo na venda
        if (sale.clientId) {
            const client = (storage.getClients() || []).find(c => String(c.id) === String(sale.clientId));
            if (client) {
        ...
        // Fallback: Se não achou pelo ID, tenta pelo nome exato
        if (!phone && sale.clientName) {
            const client = (storage.getClients() || []).find(c => c.name === sale.clientName);
            if (client) {
                phone = client.mobile || client.phone || '';
            }
        }
        ...
        if (!cleanPhone) {
            const input = prompt(`O cliente "${clientName}" não possui celular cadastrado.\n\nDigite o número do WhatsApp (com DDD):`);
            if (input) cleanPhone = input.replace(/\D/g, '');
            else return; // User cancelou
        }

        // Auto-fix BR numbers (Add 55 if missing)
        if (cleanPhone.length >= 10 && cleanPhone.length <= 11) {
            cleanPhone = '55' + cleanPhone;
        }

        const isQuote = sale.status === 'quote';
        const title = isQuote ? '📄 *ORÇAMENTO UNITECH*' : '✅ *COMPROVANTE DE VENDA*';
        const filename = `${isQuote ? 'Orcamento' : 'Comprovante'}_${sale.id}.pdf`;

        // 1. Check if libraries are available
        if (!window.jspdf || !window.html2canvas) {
            alert('Erro: Bibliotecas de PDF não carregadas.');
            return;
        }

        try {
            // 1. Session & Connectivity Check
            if (!window.whatsappService || window.whatsappService.status !== 'CONNECTED') {
                const msg = '❌ WhatsApp desconectado. Por favor, conecte o WhatsApp no menu lateral antes de enviar.';
                if (window.toastService) window.toastService.error(msg);
                else alert(msg);
                return;
            }

            if (window.toastService) window.toastService.info('Gerando PDF...');

            // 2. Generate PDF from HTML
            const html = printerService.generateCupomHTML(sale);
            ...
            // Otimização: JPEG 0.8 para reduzir tamanho do payload
            ...
            console.log(`[WhatsAppShare] PDF Size: ${(pdfBase64.length / 1024).toFixed(2)} KB`);

            // 3. Send via WhatsApp
            if (window.whatsappService && window.whatsappService.status === 'CONNECTED') {
                // Close modals to show progress/toast better
                ...
                if (window.toastService) window.toastService.info('Enviando para o WhatsApp do cliente...');

                const caption = `${title}\n*Nº:* ${sale.id}\n*Total:* R$ ${sale.total.toFixed(2)}`;

                try {
                    const response = await window.whatsappService.sendMedia(cleanPhone, pdfBase64, filename, caption);

                    if (response && response.error) {
                        throw new Error(response.error);
                    }

                    if (window.toastService) window.toastService.success('✅ Comprovante enviado com sucesso!');
                } catch (sendError) {
                    let userMsg = 'Erro ao enviar WhatsApp.';
                    const errorMsg = sendError.message || '';
                    if (errorMsg === 'INVALID_NUMBER' || errorMsg.includes('No LID for user')) {
                        userMsg = '❌ Ops! Este número não parece estar vinculado a uma conta de WhatsApp válida. Verifique o cadastro do cliente.';
                    } else if (errorMsg === 'WHATSAPP_DISCONNECTED') {
                        userMsg = '❌ Conexão com WhatsApp perdida. Reconecte e tente novamente.';
                    } else {
                        userMsg = `❌ Erro no envio: ${errorMsg}`;
                    }

                    if (window.toastService) window.toastService.error(userMsg);
                    else alert(userMsg);

                    // Re-open modal on error so user can try again or check number
                    document.getElementById('post-sale-modal')?.classList.remove('hidden');
                }
            } else {
                alert('Módulo WhatsApp não disponível ou desconectado.');
            }
        } catch (error) {
            console.error('[WhatsAppShare] Error:', error);
            alert(`Erro ao processar comprovante: ${error.message}`);
        }
    }
    ...
    showPostSaleModal(sale) {
        ...
        // WhatsApp Subtitle Update
        const whatsappSubtitle = document.getElementById('btn-share-whatsapp-subtitle');
        if (whatsappSubtitle) {
            whatsappSubtitle.innerText = 'Comprovante em PDF';
        }

        // Always show WhatsApp button
        document.getElementById('btn-share-whatsapp').classList.remove('hidden');
    ...
    attachPostSaleEvents(sale) {
        ...
        const handleWhatsApp = () => {
            this.handleWhatsAppSharing(sale);
        };
    ...
    renderHistory() {
        ...
        return `
              <div class="h-full flex flex-col bg-white rounded-lg shadow-sm border border-gray-200 animate-fade-in">
                   <div class="p-3 md:p-4 border-b border-gray-100 flex flex-col md:flex-row justify-between items-start md:items-center bg-gray-50 rounded-t-lg gap-3">
                       <h3 class="font-bold text-base md:text-lg text-gray-800 flex items-center gap-2">
                          <i data-feather="clock" class="w-4 h-4 md:w-5 md:h-5 text-gray-500"></i>
                          Histórico de Vendas
                       </h3>
                       <div class="flex items-center gap-2 md:gap-3 w-full md:w-auto">
                           <div class="relative flex-1 md:flex-none">
                               ...
                               <input type="text" id="history-search" placeholder="ID ou Cliente..." class="pl-9 pr-4 py-2 bg-white border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-unitech-primary/20 transition-all w-full md:w-64 font-medium" oninput="window.salesModule.filterHistory(this.value)">
                           </div>
                           <button onclick="window.salesModule.render();" class="text-[10px] font-black text-white bg-gray-800 hover:bg-black px-3 py-2.5 rounded flex items-center gap-2 transition-colors uppercase tracking-widest shrink-0">
                               <i data-feather="arrow-left" class="w-3.5 h-3.5"></i> <span class="hidden md:inline">VOLTAR AO PDV</span><span class="md:hidden">VOLTAR</span>
                           </button>
                       </div>
                   </div>
                   
                   <div class="flex-1 overflow-y-auto p-2 md:p-4 custom-scrollbar bg-gray-50/30">
                       <table class="hidden lg:table w-full text-left border-collapse bg-white rounded-xl overflow-hidden shadow-sm">
                          <thead>
    ...
    getSaleDetailsModalHTML() {
        return `
        <div id="sale-details-modal" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-[70] hidden flex items-center justify-center p-0 md:p-4 animate-fade-in">
             <div class="bg-white rounded-none md:rounded-2xl shadow-2xl max-w-2xl w-full overflow-hidden transform transition-all scale-100 flex flex-col h-full md:h-auto md:max-h-[90vh]">
                 <div class="bg-gray-50 p-4 md:p-5 border-b border-gray-100 flex justify-between items-center shrink-0">
                     <div>
                        <h3 class="font-black text-base md:text-xl text-gray-800 uppercase tracking-tight flex items-center gap-2">
                            <i data-feather="file-text" class="w-4 h-4 md:w-5 md:h-5 text-gray-500"></i> Detalhes da Venda
                        </h3>
                         <p class="text-[9px] md:text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-0.5" id="detail-id-display">V-000000</p>
                      </div>
                  ...
                  <div class="p-4 md:p-6 overflow-y-auto flex-1 custom-scrollbar space-y-4 md:space-y-6">
                      <!-- Info Grid -->
                ...
                <div class="p-4 bg-gray-50 border-t border-gray-100 flex flex-col md:flex-row justify-between items-center gap-3 shrink-0">
               <div class="flex flex-wrap gap-2 w-full md:w-auto justify-center">
                   <button id="btn-print-nfe-history" class="flex-1 md:flex-none px-4 py-2.5 bg-blue-50 text-blue-700 font-bold rounded-lg border border-blue-200 hover:bg-blue-100 transition-colors text-[10px] uppercase tracking-widest flex items-center justify-center gap-2">
                       <i data-feather="file-text" class="w-3.5 h-3.5"></i> NF-E
                   </button>
                   <button id="btn-print-cupom-history" class="flex-1 md:flex-none px-4 py-2.5 bg-green-50 text-green-700 font-bold rounded-lg border border-green-200 hover:bg-green-100 transition-colors text-[10px] uppercase tracking-widest flex items-center justify-center gap-2">
                       <i data-feather="printer" class="w-3.5 h-3.5"></i> CUPOM
                   </button>
                   <button id="btn-share-whatsapp-history" class="flex-1 md:flex-none px-4 py-2.5 bg-[#25D366]/10 text-green-600 font-bold rounded-lg border border-[#25D366]/20 hover:bg-[#25D366]/20 transition-colors text-[10px] uppercase tracking-widest flex items-center justify-center gap-2">
                       <i data-feather="message-circle" class="w-3.5 h-3.5"></i> WHATSAPP
                   </button>
                   <button id="btn-resume-quote" class="hidden w-full md:w-auto px-4 py-2.5 bg-orange-500 text-white font-bold rounded-lg hover:bg-orange-600 transition-colors text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg shadow-orange-500/20">
                       <i data-feather="play-circle" class="w-3.5 h-3.5"></i> CONCRETIZAR VENDA
                   </button>
               </div>
               <button onclick="document.getElementById('sale-details-modal').classList.add('hidden')" class="w-full md:w-auto px-6 py-2.5 bg-gray-900 text-white font-bold rounded-lg hover:bg-black transition-colors text-[10px] uppercase tracking-widest">FECHAR</button>
            </div>
             </div>
        </div>
        `;
    ...
    viewSaleDetails(saleId) {
        try {
            ...
            if (btnPrintNFE && btnPrintCupom) {
                ...
                const btnWhatsAppHistory = document.getElementById('btn-share-whatsapp-history');
                if (btnWhatsAppHistory) {
                    btnWhatsAppHistory.onclick = (e) => {
                        e.preventDefault();
                        this.handleWhatsAppSharing(sale);
                    };
                }
    ...
    attachEvents() {
        // POS Search
        const searchInput = document.getElementById('pos-search');
        const searchInputMobile = document.getElementById('pos-search-mobile');
        ...
        const handleSearchInput = (e) => {
            ...
            if (this.filteredProducts.length === 0 && term.length > 0) {
                ...
                if (id) {
                    ...
                    if (os) {
                        const situation = (os.situation || '').toUpperCase();
                        if (['REALIZADO', 'CONCLUÍDO', 'ENTREGUE'].includes(situation)) {
                            if (situation !== 'FATURADO' && situation !== 'PAGO') {
                                const valTotal = parseFloat(os.valTotal || 0);
                                if (valTotal > 0) {
                                    this.filteredProducts = [{
        ...
        if (searchInputMobile && !searchInputMobile.dataset.hasListener) {
            searchInputMobile.dataset.hasListener = 'true';
        ...
        // Category Filter
        const categorySelect = document.getElementById('pos-category');
        const categorySelectMobile = document.getElementById('pos-category-mobile');

        const handleCategoryChange = (e) => {
            const val = e.target.value;
            this.filteredProducts = val ?
                this.products.filter(p => p.category === val) : [...this.products];
            document.getElementById('pos-grid').innerHTML = this.renderProductGrid();

            if (categorySelect && e.target !== categorySelect) categorySelect.value = val;
            if (categorySelectMobile && e.target !== categorySelectMobile) categorySelectMobile.value = val;
        };
        ...
        if (categorySelectMobile && !categorySelectMobile.dataset.hasListener) {
            categorySelectMobile.dataset.hasListener = 'true';
            categorySelectMobile.addEventListener('change', handleCategoryChange);
        }

        // Mobile Filter Toggle
        const btnToggleFiltersMode = document.getElementById('btn-toggle-filters-pos-mobile');
        if (btnToggleFiltersMode) {
            btnToggleFiltersMode.onclick = () => {
                const container = document.getElementById('pos-mobile-filters-container');
                if (container) container.classList.toggle('hidden');
            };
        }

        // Mobile New Product Button
        const btnNewProductMobile = document.getElementById('btn-new-product-pos-mobile');
        if (btnNewProductMobile) {
            btnNewProductMobile.onclick = () => {
                document.querySelector('[data-target="storefront"]').click();
                setTimeout(() => {
                    if (window.storefrontModule) window.storefrontModule.openModal(null, 'create');
                }, 500);
            };
        }
        ...
        if (btnAddClient && !btnAddClient.dataset.hasListener) {
            btnAddClient.dataset.hasListener = 'true';
            btnAddClient.addEventListener('click', () => document.getElementById('quick-client-modal').classList.remove('hidden'));
        }
        const btnAddClientMob = document.getElementById('btn-add-client-mobile');
        if (btnAddClientMob && !btnAddClientMob.dataset.hasListener) {
            btnAddClientMob.dataset.hasListener = 'true';
            btnAddClientMob.addEventListener('click', () => document.getElementById('quick-client-modal').classList.remove('hidden'));
        }

        const btnShareCart = document.getElementById('btn-share-cart-whatsapp');
        if (btnShareCart && !btnShareCart.dataset.hasListener) {
            btnShareCart.dataset.hasListener = 'true';
            btnShareCart.addEventListener('click', () => this.shareCartCatalog());
        }
        const btnShareCartMob = document.getElementById('btn-share-cart-whatsapp-mobile');
        if (btnShareCartMob && !btnShareCartMob.dataset.hasListener) {
            btnShareCartMob.dataset.hasListener = 'true';
            btnShareCartMob.addEventListener('click', () => this.shareCartCatalog());
        }

        const btnSaveQClient = document.getElementById('btn-save-quick-client');
        if (btnSaveQClient && !btnSaveQClient.dataset.hasListener) {
            btnSaveQClient.dataset.hasListener = 'true';
            btnSaveQClient.addEventListener('click', () => this.saveQuickClient());
        }

        const clientSearch = document.getElementById('pos-client-search');
        const clientSearchMob = document.getElementById('pos-client-search-mobile');
        const handleClientInput = (e) => {
            this.handleClientSearch(e.target.value);
            const isMob = e.target.id.includes('mobile');
            const results = document.getElementById(isMob ? 'client-results-mobile' : 'client-results');
            if (results && isMob) {
        ...
        if (clientSearch) {
            clientSearch.addEventListener('input', handleClientInput);
            clientSearch.addEventListener('blur', () => setTimeout(() => document.getElementById('client-results').classList.add('hidden'), 200));
        }
        if (clientSearchMob) {
            clientSearchMob.addEventListener('input', handleClientInput);
            clientSearchMob.addEventListener('blur', () => setTimeout(() => document.getElementById('client-results-mobile').classList.add('hidden'), 200));
        }
        ...
        this._scanListener = (e) => this.handleScanListener(e);
        ...
        document.querySelectorAll('.checkout-method').forEach(btn => {
            btn.onclick = () => {
                ...
                if (method === 'CASH') {
                    ...
                    if (banks.length === 0) {
                        if (confirm('Nenhum banco cadastrado para PIX. Continuar?')) this.processSale('PIX');
                    } else {
                        document.getElementById('pix-payment-details').classList.remove('hidden');
                        document.getElementById('checkout-methods-grid').classList.add('hidden');
                    }
                } else if (method === 'ORCAMENTO') {
                    if (confirm('Salvar como Orçamento?')) this.processSale(method);
                } else {
        ...
        if (btnConfirmPix) btnConfirmPix.onclick = () => this.confirmPix();

        const btnConfirmCash = document.getElementById('btn-confirm-cash');
        if (btnConfirmCash) btnConfirmCash.onclick = () => this.confirmCash();

        const btnConfirmInstallments = document.getElementById('btn-confirm-installments');
        if (btnConfirmInstallments) btnConfirmInstallments.onclick = () => this.confirmInstallments();
    }

    handleEnterKey(value) {
        const term = this.lastScannedCode || value.trim();
        ...
        if (product) {
            ...
            if (!handled) {
                if (window.toastService) window.toastService.error('Nenhum produto ou OS encontrado');
                this.lastScannedCode = null;
            }
        }
    }

    handleScanListener(e) {
        ...
        if (e.key === 'Enter' && this.lastScannedCode && !isInput) {
            e.preventDefault();
            this.handleEnterKey(this.lastScannedCode);
            return;
        }

        if (isInput && activeId !== 'pos-search' && activeId !== 'pos-search-mobile') return;
        ...
        if (e.key.length === 1) {
            ...
            if (this.buffer.length >= 3) {
                ...
                if (product) {
                    ...
                    setTimeout(() => {
                        const productCard = document.querySelector(`[onclick*="addToCart('${product.id}')"]`);
                        if (productCard) productCard.classList.add('ring-4', 'ring-unitech-primary');
                    }, 100);
                } else {
                    if (window.toastService) window.toastService.error(`Produto não encontrado: ${this.buffer}`);
                }
            }
            this.buffer = '';
        }
    ...
    attachGlobalShortcuts() {
        document.addEventListener('keydown', (e) => {
            if (e.key === 'F5') return; // Allow Native Refresh

            if (e.key === 'Escape') {
                document.getElementById('mobile-cart-modal')?.classList.add('hidden');
                document.getElementById('checkout-modal')?.classList.add('hidden');
                document.getElementById('quick-client-modal')?.classList.add('hidden');
                document.getElementById('sale-details-modal')?.classList.add('hidden');
            }
        });
    }
    ...
    renderIdleScreen() {
        ...
        return `
        <div class="col-span-full h-full min-h-[450px] flex flex-col items-center justify-center p-8 bg-[#fff2e6] dark:bg-slate-950 rounded-[2.5rem] border border-orange-100/50 dark:border-slate-800 shadow-[xl_inset] animate-fade-in relative overflow-y-auto custom-scrollbar group">
            ...
            <div class="absolute bottom-0 left-0 w-full leading-[0] transform translate-y-[2px]">
                <svg viewBox="0 0 1440 240" preserveAspectRatio="none" class="w-full h-40 md:h-56 filter drop-shadow-[0_-5px_15px_rgba(255,255,255,0.8)]">
                    <path fill="currentColor" class="text-white dark:text-slate-950 transition-colors duration-500" fill-opacity="1" d="M0,160L60,144C120,128,240,96,360,101.3C480,107,600,149,720,149.3C840,149,960,107,1080,90.7C1200,75,1320,85,1380,90.7L1440,96L1440,320L1380,320C1320,320,1200,320,1080,320C960,320,840,320,720,320C600,320,480,320,360,320C240,320,120,320,60,320L0,320Z"></path>
                </svg>
            </div>
            ...
            <div class="z-10 relative mb-6 md:mb-10 text-center">
                <div class="relative bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl px-8 py-6 md:px-16 md:py-10 rounded-[2.5rem] md:rounded-[3.5rem] border border-white dark:border-slate-800 shadow-[0_20px_60px_rgba(238,68,68,0.08)] dark:shadow-none transition-all">
                    <h1 class="text-4xl md:text-8xl font-black leading-none tracking-tighter text-transparent bg-clip-text bg-gradient-to-b from-gray-900 to-gray-600 dark:from-white dark:to-slate-400 mb-2">
                        CAIXA
                    </h1>
                    <h1 class="text-4xl md:text-8xl font-black leading-[0.8] tracking-tighter text-transparent bg-clip-text bg-gradient-to-t from-unitech-primary to-orange-600 flex items-center justify-center gap-3 md:gap-6">
                        LIVRE
                        <span class="relative flex h-3 w-3 md:h-4 md:w-4">
                            ...
                            <span class="relative inline-flex rounded-full h-3 w-3 md:h-4 md:w-4 bg-unitech-primary shadow-[0_0_15px_rgba(238,68,68,0.6)]"></span>
                        </span>
                    </h1>
                </div>
            </div>
            ...
            <div class="z-10 flex flex-col items-center gap-4">
                <div class="relative flex flex-col items-center">
                    <span id="idle-clock" class="text-5xl md:text-[6rem] font-black text-[#1a1a1a] dark:text-white dark:drop-shadow-[0_0_25px_rgba(255,255,255,0.25)] tracking-tight tabular-nums drop-shadow-sm leading-none transition-all duration-300">
                        ${currentTime}
                    </span>
