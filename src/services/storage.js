...
export class StorageService {
    ...
    /**
     * CENTRAL SAAS VALIDATION (Middleware)
     * Checks if tenant is active and has storage capacity
     */
    async validateTenantAccess(operationType = 'write', estimatedSizeHandled = 0) {
        const tenantId = this.getCurrentTenantId();
        if (tenantId === 'master') return true; // Master bypasses all checks

        const tenants = TenantRepository.getAll(true); // Bypass to see all tenants
        ...
        if (tenant.status === 'suspended') {
            throw new Error('Acesso Suspenso: Regularize sua assinatura no painel financeiro.');
        }

        if (tenant.status === 'canceled') {
            throw new Error('Conta Cancelada: Entre em contato com o suporte Master.');
        }

        // 2. Check Storage (Only for write/add operations)
        if (operationType === 'write') {
            const limitBytes = (tenant.storageLimitGB || 5) * 1024 * 1024 * 1024;
            const currentUsage = tenant.currentUsageBytes || 0;

            if (currentUsage + estimatedSizeHandled > limitBytes) {
                throw new Error('Limite de Armazenamento atingido! Faça upgrade do seu plano para continuar.');
            }
        }
    ...
    /**
     * Estimated size of an object in bytes
     */
    getObjectSize(obj) {
     ...
     */
    async updateTenantUsage(bytesDiff) {
        const tenantId = this.getCurrentTenantId();
        if (tenantId === 'master') return;

        const tenant = TenantRepository.getAll(true).find(t => t.id === tenantId);
        if (tenant) {
            await TenantRepository.update(tenantId, {
                currentUsageBytes: (tenant.currentUsageBytes || 0) + bytesDiff
            });
        }
    }
    ...
    async addProduct(product) {
        const size = this.getObjectSize(product);
        await this.validateTenantAccess('write', size);
        const result = await ProductRepository.add(product);
        if (result) await this.updateTenantUsage(size);
        return result;
    }
    ...
    async addClient(client) {
        ...
        const size = this.getObjectSize(client);
        await this.validateTenantAccess('write', size);
    ...
    async addSale(sale) {
        const size = this.getObjectSize(sale);
        await this.validateTenantAccess('write', size);
        const result = await SaleRepository.add(sale);
        if (result) {
            await this.updateTenantUsage(size);
    ...
    async addChecklist(checklist) {
        const size = this.getObjectSize(checklist);
        await this.validateTenantAccess('write', size);
        const result = await ChecklistRepository.add(checklist);
        if (result) await this.updateTenantUsage(size);
    ...
    // --- Bulk Operations ---
    async bulkDelete(types) {
        const tenantId = this.getCurrentTenantId();
        const results = {};

        if (types.includes('financial')) {
            const txs = TransactionRepository.getAll();
            for (const t of txs) await TransactionRepository.delete(t.id);
            results.financial = txs.length;
        }

        if (types.includes('sales')) {
            const sales = SaleRepository.getAll();
            for (const s of sales) await SaleRepository.delete(s.id);
            results.sales = sales.length;
        }

        if (types.includes('clients')) {
            const clients = ClientRepository.getAll();
            for (const c of clients) await ClientRepository.delete(c.id);
            results.clients = clients.length;
        }

        if (types.includes('inventory')) {
            const products = ProductRepository.getAll();
            for (const p of products) await ProductRepository.delete(p.id);
            results.inventory = products.length;
        }

        if (types.includes('os')) {
            const checklists = ChecklistRepository.getAll();
            for (const c of checklists) await ChecklistRepository.delete(c.id);
            const warranties = WarrantyRepository.getAll();
            for (const w of warranties) await WarrantyRepository.delete(w.id);
            results.os = (checklists.length || 0) + (warranties.length || 0);
        }

        audit.log('SYSTEM_RESET', 'User', tenantId, results);
        return results;
    }

    async clearFinancialData() {
        return this.bulkDelete(['financial', 'sales']);
    }
}
